...
data1 = pd.read_csv(BASE_DIR / "relational-dataset/fodors-zagats/tableA.csv")
data1 = np.array(data1)
# data1 = np.nan_to_num(data1, nan=None)
data2 = pd.read_csv(BASE_DIR / "relational-dataset/fodors-zagats/tableB.csv")
data2 = np.array(data2)
# data2 = np.nan_to_num(data2, nan=None)
data3 = pd.read_csv(BASE_DIR / "relational-dataset/fodors-zagats/matches.csv")
...
outfile = open(BASE_DIR / 'network/restaurant.txt', 'w', encoding='utf-8')
for i in range(len(entity_type1)):
...
outfile = open(BASE_DIR / 'network/address.txt', 'w', encoding='utf-8')
for i in range(len(entity_type2_new)):
...
outfile = open(BASE_DIR / 'network/city.txt', 'w', encoding='utf-8')
for i in range(len(entity_type3)):
...
address_city = []
outfile = open(BASE_DIR / 'network/restaurant-address.txt', 'w', encoding='utf-8')
for i in range(len(entity_type1)):
...
outfile = open(BASE_DIR / 'network/address-city.txt', 'w', encoding='utf-8')
...
def delete_type_add_symbol():
    ...
    ary_address=[]
    ary_city=[]
    with open(BASE_DIR / "network/restaurant.txt",'r',encoding='utf-8') as f:
        for line in f:
            line = line.strip('\n').split('\t')
    ...
    with open(BASE_DIR / "network/address.txt",'r',encoding='utf-8') as f:
        for line in f:
            line = line.strip('\n').split('\t')
    ...
    with open(BASE_DIR / "network/city.txt",'r',encoding='utf-8') as f:
        for line in f:
            line = line.strip('\n').split('\t')
